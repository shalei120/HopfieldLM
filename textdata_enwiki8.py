...
class TextData_enwiki8:
    ...
    def loadCorpus(self, corpusname):
        ...
        """
        if args['server'] == 'dgx':
            self.basedir = './.data/enwiki8'
        else:
            self.basedir = '../enwiki8'

        self.corpus = self.basedir


        self.fullSamplesPath = args['rootDir'] + '/LMdata_enwiki8.pkl'  # Full sentences length/vocab
        ...
        if not datasetExist:  # First time we load the database: creating all files
            ...
            with open(args['rootDir'] + "/voc_enwiki8.txt", "w") as v:
            ...
            self.word2index = self.read_word2vec(args['rootDir'] + '/voc_enwiki8.txt')
            ...
            dataset['train'] = [(self.TurnWordID(sen), sen) for sen in tqdm(dataset['train'])]
            total_num = len(dataset['train'])
            dataset['test']= dataset['train'][int(0.8*total_num):]
            dataset['dev']= dataset['train'][int(0.7*total_num):int(0.8*total_num)]
            dataset['train']= dataset['train'][:int(0.7*total_num)]
